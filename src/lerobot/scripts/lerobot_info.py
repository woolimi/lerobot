...
def get_sys_info() -> dict[str, str]:
    ...
    torch_mps_available = "N/A"
    torch_xpu_available = "N/A"
    cuda_version = "N/A"
    gpu_model = "N/A"
    try:
        import torch
        ...
        torch_xpu_available = getattr(torch, "xpu", None) is not None and torch.xpu.is_available()
    ...
    # Accelerate device auto-detection (best-effort)
    accelerate_version = "N/A"
    accelerate_device = "N/A"
    try:
        import accelerate

        accelerate_version = str(getattr(accelerate, "__version__", "Installed (version not found)"))
        from accelerate import Accelerator

        accelerate_device = str(Accelerator().device)
    except ImportError:
        pass
    except Exception:
        # Any runtime errors should not prevent printing system info.
        accelerate_device = "Installed (device detection failed)"

    info.update(
        {
            "PyTorch version": torch_version,
            "CUDA available (torch.cuda.is_available)": str(torch_cuda_available),
            "MPS available (torch.backends.mps.is_available)": str(torch_mps_available),
            "XPU available (torch.xpu.is_available)": str(torch_xpu_available),
            "Cuda version": cuda_version,
            "GPU model": gpu_model,
            "Accelerate version": accelerate_version,
            "Accelerate detected device": accelerate_device,
        }
    )
    scripts = "N/A"
    try:
