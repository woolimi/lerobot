...
# 훈련 시 03.train_act_pilot.sh 에서 REPO_ID/DATASET_ROOT 를 merged 결과로 지정하면 됨
#
# 사용법 (프로젝트 루트에서):
#   ./scripts/05.merge.sh
#
...
#   MERGED_REPO_ID  merge 결과 repo_id (기본: woolim/merged). 비우면 입력 프롬프트
#
# 종료: Ctrl+C
...
if [[ -z "$MERGED_REPO_ID" ]]; then
  echo -n "merge 결과 repo_id (기본: woolim/merged): "
  read -r MERGED_REPO_ID
  MERGED_REPO_ID="${MERGED_REPO_ID:-woolim/merged}"
fi
...
echo ""
echo "훈련 시 예: REPO_ID=$MERGED_REPO_ID DATASET_ROOT=$DATA_DIR/${MERGED_REPO_ID//\//_} ./scripts/03.train_act_pilot.sh"
